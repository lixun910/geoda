...
OutputDir=..\..
OutputBaseFilename=GeoDa_1.22.1_arm64_Setup
;OutputDir=userdocs:Inno Setup Examples Output

; "ArchitecturesAllowed=arm64" specifies that Setup cannot run on
; anything but ARM 64-bit processors.
ArchitecturesAllowed=arm64

; "ArchitecturesInstallIn64BitMode=arm64" requests that the install be
; done in "64-bit mode" on ARM64, meaning it should use the native
...
;ARM64 Mode
Root: "HKLM"; Subkey: "SOFTWARE\Microsoft\Internet Explorer\MAIN\FeatureControl\FEATURE_BROWSER_EMULATION"; ValueType: dword; ValueName: "GeoDa.exe"; ValueData: "$2AF9"; Check: IsArm64
;run as admin
...
begin
  Result := not RegKeyExists(HKLM,'SOFTWARE\Microsoft\VisualStudio\14.0\VC\Runtimes\ARM64');
end;
...
[Run]
Filename: {app}\VC_redist.arm64.exe; StatusMsg: Installing Visual C++ Redistributable for Visual Studio 2019 (14.28.29913.0)...; Check: VCRedistNeedsInstall
