...
if ! type "g++" > /dev/null; then
    echo "You need to install g++/gcc version 6 to run this script."
...
MAKER="make CC=gcc-5 CPP=g++-5 CXX=g++-5 LD=g++-5 -j $CPUS"
export CFLAGS="-m64"
export CXXFLAGS="-m64"
...
#########################################################################
install_library c-ares-1.10.0 https://c-ares.haxx.se/download/c-ares-1.10.0.tar.gz libcares.a
...
LIB_NAME=curl-7.61.0
LIB_CHECKER=libcurl.a
LIB_URL=https://curl.haxx.se/download/curl-7.61.0.tar.gz
LIB_FILENAME=curl-7.61.0.tar.gz
echo $LIB_NAME
...
# install GEOS
#########################################################################
install_library geos-3.6.2 https://download.osgeo.org/geos/geos-3.6.2.tar.bz2  libgeos.a
...
echo "%%%%%%%%%%%%%%%%%%%%%%%%"
{
	...
	echo "$LIB_CHECKER not found.  Building Boost..."
        ...
        ./bootstrap.sh
        chmod +x b2
        ./b2 --toolset=gcc-5 --with-thread --with-date_time --with-chrono --with-system link=static threading=multi stage
    fi
...
# sudo apt-get install libgtk2.0-dev libglu1-mesa-dev libgl1-mesa-dev
{
    ...
    if ! [ -f "$PREFIX/bin/$LIB_CHECKER" ] ; then
        ...
        CC=gcc-5 CXX=g++-5 CPP=g++-5 LD=g++-5 ./configure --with-gtk=2 --enable-ascii --disable-shared --disable-monolithic --with-opengl --enable-postscript --without-libtiff --disable-debug --enable-webview --prefix=$PREFIX
        $MAKE
        make install
        cd ..
    fi
