...
// Fragment shader for the circles
...
// myOGLCircles
// ----------------------------------------------------------------------------
myOGLCircles::myOGLCircles()
{
    m_circleVAO = m_bufVertId = m_bufColNorId = m_bufIndexId = 0;
    m_circleShaders = NULL;
}
...
void myOGLCircles::Clear()
{
    if ( m_circleShaders )
        m_circleShaders->DisableGenericVAA();
    ...
    m_circleShaders = NULL;
    m_circleVAO = m_bufIndexId = m_bufColNorId = m_bufVertId = 0;
}

void myOGLCircles::SetBuffers(myOGLShaders* theShader,
                                GLsizei nuPoints, GLsizei nuCircles,
                                ...
                                const GLfloat* norm, const GLushort* indices)
{
    ...
    nBytes = (nuPoints * 4 + nuPoints * 3) * sizeof(GLfloat);
    ...
    nBytes = nuPoints * 3 * sizeof(GLfloat);
    ...
    nBytes = nuPoints * sizeof(GLushort); //Each circle needs 1 indice
    ...
    m_nuCircles = nuCircles;
    m_circleShaders = theShader;
...
void myOGLCircles::Draw(const GLfloat* unifMvp, const GLfloat* unifToVw,
                          const myLight* theLight)
{
    ...
    glUniformMatrix4fv(m_circleShaders->GetUnifLoc("mMVP"), 1, GL_FALSE, unifMvp);
    glUniformMatrix4fv(m_circleShaders->GetUnifLoc("mToViewSpace"), 1, GL_FALSE, unifToVw);
    // Pass the light, in View coordinates in this sample
    glUniform4fv(m_circleShaders->GetUnifLoc("lightProps"), 1, theLight->GetFLightPos());
    glUniform3fv(m_circleShaders->GetUnifLoc("lightColour"), 1, theLight->GetFLightColour());
...
void myOGLManager::SetShadersAndTriangles()
{
    ...
    m_CircleShaders.AddAttrib("in_Position");
    m_CircleShaders.AddAttrib("in_Colour");
    m_CircleShaders.AddAttrib("in_Normal");
    m_CircleShaders.AddUnif("mMVP");
    m_CircleShaders.AddUnif("mToViewSpace");
    m_CircleShaders.AddUnif("lightProps");
    m_CircleShaders.AddUnif("lightColour");
    m_CircleShaders.AddCode(circleVertexShader, GL_VERTEX_SHADER);
    m_CircleShaders.AddCode(illuminationShader, GL_FRAGMENT_SHADER);
    ...
    GLfloat* gPoints = new GLfloat[n*3]; // x,y,z
    GLfloat* gPointColors = new GLfloat[n*4]; // rgba
    GLfloat* gPointNormals = new GLfloat[n*3]; //
    GLushort* gPointIndices = new GLushort[n];
    
    for (int i=0; i<n; i++ ) {
        float x = 100.0 * rand() / RAND_MAX;
        float y = 100.0 * rand() / RAND_MAX;
        //float z = 100.0 * rand() / RAND_MAX;
        gPoints[i*3] = x;
        gPoints[i*3 + 1] = y;
        gPoints[i*3 + 2] = 0;
        
        gPointColors[i*3] = 0.0;
        gPointColors[i*3+1] = 0.0;
        gPointColors[i*3+2] = 0.0;
        gPointColors[i*3+3] = 1.0;
       
        // -0.81650f, 0.33333f,  0.47140f
        gPointNormals[i*3] = 0;
        gPointNormals[i*3+1] = 0;
        gPointNormals[i*3+2] = 1;
        
        gPointIndices[i] = i;
    }
    m_Circles.SetBuffers(&m_CircleShaders, n, n, gPoints, gPointColors, gPointNormals, gPointIndices);
}
...
void myOGLManager::Render()
{
    ...
    glClearColor((GLfloat)1.0, (GLfloat)1.0, 1.0, (GLfloat)1.0);
    glClear( GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT );

    m_Circles.Draw(m_Camera.GetFloatMVP(), m_Camera.GetFloatToVw(), &m_Light);
